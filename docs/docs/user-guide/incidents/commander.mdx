...
- [`/dispatch-add-timeline-event`](#dispatch-add-timeline-event)
- [`/dispatch-assign-role`](#dispatch-assign-role)
- [`/dispatch-engage-oncall`](#dispatch-engage-oncall)
- [`/dispatch-list-incidents`](#dispatch-list-incidents)
- [`/dispatch-list-my-tasks`](#dispatch-list-my-tasks)
- [`/dispatch-list-participants`](#dispatch-list-participants)
- [`/dispatch-list-tasks`](#dispatch-list-tasks)
- [`/dispatch-list-workflows`](#dispatch-list-workflows)
- [`/dispatch-notifications-group`](#dispatch-notifications-group)
- [`/dispatch-report-executive`](#dispatch-report-executive)
- [`/dispatch-report-incident`](#dispatch-report-incident)
- [`/dispatch-report-tactical`](#dispatch-report-tactical)
- [`/dispatch-update-incident`](#dispatch-update-incident)
- [`/dispatch-update-participant`](#dispatch-update-participant)
- [`/dispatch-run-workflow`](#dispatch-run-workflow)
- [`/dispatch-create-task`](#dispatch-create-task)
- [`/dispatch-create-case`](#dispatch-create-case)
- [`/dispatch-update-case`](#dispatch-update-case)
- [`/dispatch-escalate-case`](#dispatch-escalate-case)
- [`/dispatch-engage-user`](#dispatch-engage-user)
- [`/dispatch-list-signals`](#dispatch-list-signals)
...
{/* The sections below have no screenshot yet; capturing one needs a live Slack workspace. See issue #135. */}
...
A case cannot be escalated from here — the status select deliberately omits `Escalated`. Use [`/dispatch-escalate-case`](#dispatch-escalate-case) instead, which creates the incident the case becomes.
...
Use `/dispatch-list-incidents` to open a modal listing every incident across every project that is currently active or stable, plus any incident closed in the last 24 hours. Each entry links to the incident in Dispatch and shows its commander, project, status, type, severity, and priority.

The command takes no arguments and can be run outside an incident channel as well as inside one. Restricted incidents are never listed, and the modal shows at most 49 incidents; use the Dispatch web interface if your deployment has more.

### /dispatch-list-signals

Use `/dispatch-list-signals` to see which signal definitions send their cases to the conversation you run it from. It opens a modal listing every definition whose case type targets that conversation, showing each one's name and variant alongside a [Snooze](../../administration/settings/signal/filter.mdx#snooze-filter) button that temporarily stops it creating cases. Longer lists are paged 25 at a time.
