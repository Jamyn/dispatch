...
On the same `Interactivity & Shortcuts` page, under `Select Menus`, set the `Options Load URL` to `/api/v1/<organization>/events/slack/menu`. Replace `<organization>` with your organization's name. The default one is `default`.

This is a separate field from the `Request URL` above and does not inherit from it. Dispatch uses it for the type-ahead menus that load their choices on demand rather than embedding them in the modal: tag selection, and project selection in deployments with more than 100 enabled projects. If it is unset, those menus open empty and stay empty, and because the field is required the form cannot be submitted.
...
| `/dispatch-list-incidents [project name]`                                           | Lists current active and stable incidents and closed incidents in the last 24 hours, for one project or for all of them.    |
...
| `/dispatch-report-incident`                                                         | Opens a modal to report an incident. This command can be run from non-incident channels where the Dispatch bot is a member. |
...
| `/dispatch-update-case`                                                             | Opens a modal to update the case. Run from a case channel.                                                                  |
| `/dispatch-escalate-case`                                                           | Opens a modal to escalate the case to an incident. Run from a case channel.                                                 |
| `/dispatch-engage-user`                                                             | Opens a modal to ask a case participant to confirm their identity with MFA. Run from a case channel.                        |
