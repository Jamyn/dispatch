...
:::info
Dispatch requires `python 3.11+` (the shipped image runs 3.14).
:::
...
Dispatch uses [Vite](https://vitejs.dev/) to build its single-page app \(SPA\) and the [Vuetify](https://vuetifyjs.com/en/) framework for material based components.
...
:::info
To correctly build its components Dispatch requires node 12.7.0+
:::
